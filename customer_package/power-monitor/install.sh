...
apt-get update -qq
apt-get install -y -qq python3-pip python3-tk mariadb-server > /dev/null 2>&1
...
# Step 4: Setup configuration file first (must exist before MySQL wizard)
...
python3 "$INSTALL_DIR/configure_mysql.py" || {
    echo -e "${YELLOW}[WARN]${NC} MySQL auto-config skipped. Edit $INSTALL_DIR/config.json manually if needed."
}
echo -e "${GREEN}[OK]${NC} MySQL step complete"
